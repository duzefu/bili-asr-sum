...
- 🎙️ 支持三个 ASR 后端，可通过配置切换：
  - **阿里云 DashScope FunASR**（`paraformer-v2`）
  - **火山引擎豆包语音识别**（大模型录音文件极速版）
  - **本地 SenseVoice-Small**（离线，无需 API Key，Apache 2.0）
...
| 变量 | 必须 | 说明 |
|---|---|---|
| `ASR_PROVIDER` | 是 | `aliyun`、`volcengine` 或 `local_sensevoice` |
...
| `LOCAL_ASR_DEVICE` | 否 | 本地 ASR 推理设备，默认 `cpu`（可填 `cuda:0`、`mps`） |
| `LOCAL_ASR_LANGUAGE` | 否 | 本地 ASR 语言，默认 `auto`（可填 `zh`、`en` 等） |
...
└── asr/
    ...
    └── local_sensevoice.py # 本地 SenseVoice 离线 ASR
tests/
├── conftest.py
├── test_downloader.py
...
- 🖥️ 本地 SenseVoice 首次运行会自动从 ModelScope 下载模型（约 500MB），需安装 `funasr` 等依赖（`pip install funasr modelscope torch torchaudio`）
